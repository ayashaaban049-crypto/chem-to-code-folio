...
export const Navbar = () => {
  ...
  const links = [
    ...
    { href: "#freelancing", label: t.nav.freelancing },
  ...
  const handleToggleLang = () => {
    ...
    const nextT = next === "ar"
      ? { title: "تم تغيير اللغة", desc: "تم تفعيل الوضع العربي — التخطيط الآن من اليمين إلى اليسار." }
      : { title: "Language switched", desc: "English mode enabled." };
    toast(nextT.title, { description: nextT.desc, duration: 2800 });
  };
    ...
    >
      <div
        className={cn(
          ...
          scrolled ? "border-border/70 bg-surface/70 backdrop-blur-xl py-2 shadow-elevated" : "border-transparent bg-transparent py-2",
        )}
      >
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="nav-underline rounded-full px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:text-foreground">
              {l.label}
            </a>
          ))}
        </nav>
      ...
      {open && (
        <div className="container-tight mt-2 lg:hidden">
          <div className="rounded-2xl border border-border/70 bg-surface/90 p-3 backdrop-blur-xl">
            {links.map((l) => (
              <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="block rounded-xl px-4 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-surface-elevated hover:text-foreground">
                {l.label}
              </a>
            ))}
            <button
              onClick={() => { setOpen(false); handleToggleLang(); }}
